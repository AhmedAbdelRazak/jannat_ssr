...
export const isActiveOrUpcomingDeal = (from, to) => {
	const today = dayjs().startOf("day");
	const fromDate = dayjs(from);
	const toDate = dayjs(to);
	if (toDate.isValid() && toDate.isBefore(today, "day")) return false;
	if (fromDate.isValid() && fromDate.isSame(today, "day")) return true;
	if (fromDate.isValid() && fromDate.isAfter(today, "day")) return true;
	if (toDate.isValid() && toDate.isSame(today, "day")) return true;
	if (toDate.isValid() && toDate.isAfter(today, "day")) return true;
	return false;
};
	...
	(Array.isArray(hotel?.roomCountDetails) ? hotel.roomCountDetails : [])
		.map((room) => {
			const offers = (Array.isArray(room?.offers) ? room.offers : [])
				.map(normalizeOffer)
				.filter((deal) => deal.base > 0 || deal.root > 0)
				.filter((deal) => isActiveOrUpcomingDeal(deal.from, deal.to));
			const monthly = (Array.isArray(room?.monthly) ? room.monthly : [])
				.map(normalizeMonthlyDeal)
				.filter((deal) => deal.monthBase > 0 || deal.monthRoot > 0)
				.filter((deal) => isActiveOrUpcomingDeal(deal.from, deal.to));
...
export const dealTotalSar = (deal = {}, commission = 0.1) => {
	if (deal.type === "monthly") {
		return safeNumber(deal.monthBase, 0) + safeNumber(deal.monthRoot, 0) * commission;
	}
	return safeNumber(deal.base, 0) + safeNumber(deal.root, 0) * commission;
};

export const dealNightlySar = (deal = {}, commission = 0.1, nights = 1) =>
	dealTotalSar(deal, commission) / Math.max(1, safeNumber(nights, 1));

const splitMoneyAcrossNights = (amount = 0, nights = 1) => {
	const count = Math.max(1, safeNumber(nights, 1));
...
export const buildDealPricingRows = (deal = {}, commission = 0.1, checkIn, checkOut) => {
	...
	const totalBase = deal.type === "monthly" ? safeNumber(deal.monthBase, 0) : safeNumber(deal.base, 0);
	const totalRoot = deal.type === "monthly" ? safeNumber(deal.monthRoot, 0) : safeNumber(deal.root, 0);
	const baseParts = splitMoneyAcrossNights(totalBase, nights);
	const rootParts = splitMoneyAcrossNights(totalRoot, nights);
	const totalParts = splitMoneyAcrossNights(totalBase + totalRoot * commission, nights);
