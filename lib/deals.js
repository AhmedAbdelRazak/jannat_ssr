...
import { generateDateRange, safeNumber } from "./booking.js";
import {
...
const parseStoredHijriDate = (value = "") => {
	const match = digitsToEnglish(value).trim().match(/^(1[34]\d{2})-(\d{1,2})-(\d{1,2})$/);
...
export const parseHijriDealRange = (text = "", options = {}) => {
	const storedStart = parseStoredHijriDate(options.fromHijri);
	const storedEnd = parseStoredHijriDate(options.toHijri);
	const mentions = storedStart && storedEnd
		? [storedStart, storedEnd]
		: extractHijriDateMentions(text);
	if (mentions.length < 2 || mentions.some((mention) => !mention.year)) return null;
...
export const normalizeOffer = (offer = {}) => {
	...
	const from = normalizeDealDate(offer?.offerFrom || offer?.from || offer?.validFrom);
	const to = normalizeDealDate(offer?.offerTo || offer?.to || offer?.validTo);
	const hijriRange = offer?.offerFromHijri && offer?.offerToHijri
		? parseHijriDealRange("", {
				fromHijri: offer.offerFromHijri,
				toHijri: offer.offerToHijri,
			})
		: null;
	return {
		type: "offer",
...
export const roomDealGroups = (hotel = {}, { today = new Date() } = {}) =>
	(Array.isArray(hotel?.roomCountDetails) ? hotel.roomCountDetails : [])
		.map((room) => {
			const offers = (Array.isArray(room?.offers) ? room.offers : [])
				...
				.filter((deal) => isFullyUpcomingDeal(deal, { today }));
...
export const resolveDealStay = (deal = {}, fallbackDates = {}) => {
	...
	if (rangeNights >= 1 && rangeNights <= maxFixedNights && fromYear <= currentYear + 5) {
		return {
			...
			hijriRange: deal.hijriRange || null,
		};
	}
