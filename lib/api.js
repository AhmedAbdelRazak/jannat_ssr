...
import { ratingAwareFetchOptions } from "./ratingFetchOptions";

export { ratingAwareFetchOptions } from "./ratingFetchOptions";
...
export async function getHotels({ freshRatings = false } = {}) {
	const data = await fetchJson(
		"/active-hotel-list",
		ratingAwareFetchOptions(freshRatings, 60)
	);
	return Array.isArray(data) ? data : [];
}

export async function getFeaturedHotels({ freshRatings = false } = {}) {
	const data = await fetchJson(
		"/active-hotels",
		ratingAwareFetchOptions(freshRatings, 60)
	);
...
export async function getRoomSearchResults(
	query,
	{ freshRatings = false } = {}
) {
	const data = await fetchJson(
		`/room-query-list/${encodeURIComponent(query)}`,
		ratingAwareFetchOptions(freshRatings, 30)
	);
	return Array.isArray(data) ? data : [];
}

export async function getDealHotels({ fresh = false } = {}) {
	const data = await fetchJson(
